...
def main(args):
    ...
    scheduler = ReduceLROnPlateau(
        optimiser, 'min', patience=config["SCHEDULER_PATIENCE"], factor=0.1, min_lr=config["MIN_LR"])
    ...
    dot = make_dot(out, params=dict(
        list(cnn.named_parameters()) + [('input', x)]))
    dot.render('cnn_architecture', format='png',
               directory=f'logs/fsc22_{timestamp}/metadata')

    model = train(cnn, train_data_loader, val_data_loader, loss_fn, optimiser, scheduler,
                  config["DEVICE"], config["EPOCHS"], timestamp, early_stopping_patience=config["EARLY_STOPPING_PATIENCE"], min_delta=config["MIN_DELTA"])

    test(model, test_data_loader, loss_fn, config["DEVICE"], "test", timestamp)

    test(model, train_data_loader, loss_fn,
         config["DEVICE"], "train", timestamp)

    test(model, val_data_loader, loss_fn, config["DEVICE"], "val", timestamp)

    torch.save(cnn.state_dict(),
               f'logs/fsc22_{timestamp}/best_model/best_model.pth')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Audio classification with CNNs')
    parser.add_argument('--architecture', type=str,
                        required=True, help='CNN architecture to use')
