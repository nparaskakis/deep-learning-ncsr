...
def test(model, data_loader, loss_fn, device, subset_name, num_classes, timestamp):
    ...
    if timestamp != None:
        cm_df.to_csv(f"logs/urb_{timestamp}/metadata/{subset_name}_confusion_matrix.csv", index=True)
    ...
    if timestamp != None:
        with open(f"logs/urb_{timestamp}/metadata/eval_on_{subset_name}_set.txt", 'w') as file:
