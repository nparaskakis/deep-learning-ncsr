...
def augment_and_save_data(audio_dir: str, metadata_file: str, output_dir: str, percentage_increase: float):
    ...
    for index, row in annotations.iterrows():
        ...
        output_audio_path = os.path.join(augmented_audio_dir, row['Dataset File Name'])
    ...
    for class_id, count in class_counts.items():
        ...
        if len(class_samples) > 0:
            num_samples_to_generate = total_samples_per_class[class_id] - class_counts[class_id]
            selected_samples = class_samples.sample(n=min(num_samples_to_generate, len(class_samples)), replace=False)
        else:
            continue
    ...
    augmented_metadata_file = os.path.join(output_dir, 'augmented_metadata.csv')
...
if __name__ == "__main__":
    ...
    augment_and_save_data(audio_dir, metadata_file, output_dir, percentage_increase)